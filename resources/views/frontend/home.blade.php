...
@section('meta_title', \App\Helpers\Cms::setting('home_meta_title', \App\Helpers\Cms::siteName()))
@section('meta_description', \App\Helpers\Cms::setting('home_meta_description', \App\Helpers\Cms::defaultMetaDescription()))
...
<section class="hero-wrap" id="kenburnsSliderContainer" data-overlay-dark="4">
    ...
    <div class="hero-content">
        <div class="container">
            <h5>{{ \App\Helpers\Cms::setting('home_hero_subtitle', 'Trekking Paradise · Cultural Diversity · Panoramic Views') }}</h5>
            <h1>{{ \App\Helpers\Cms::setting('home_hero_title', 'Travel: A companion that collects') }} <span>{{ \App\Helpers\Cms::setting('home_hero_span', 'memories') }}</span></h1>
            <p>{{ \App\Helpers\Cms::setting('home_hero_tagline', 'For every trek and adventure, there is Annapurna Region!') }}</p>
            ...
            <div class="search-panel">
                ...
                <div class="search-bar-wrap">
                    ...
                    <div class="search-bar-field">
                        ...
                        <select id="heroDestination">
                            ...
                            @foreach($activeTreks->take(10) as $t)
                            <option value="{{ route('trek-routes.index') }}?search={{ urlencode($t->name) }}">{{ $t->name }}</option>
                            @endforeach
                            @if($activeTreks->isEmpty())
                            <option value="{{ route('trek-routes.index') }}?search=Annapurna+Circuit">Annapurna Circuit</option>
                            <option value="{{ route('trek-routes.index') }}?search=Annapurna+Base+Camp">Annapurna Base Camp</option>
                            <option value="{{ route('trek-routes.index') }}?search=Poon+Hill">Poon Hill</option>
                            @endif
                        </select>
                    </div>
                    <div class="search-bar-field">
                ...
                <div class="cat-pills" id="heroPills">
                    <span>Popular:</span>
                    @foreach($popularTreks->take(5) as $t)
                    <a href="{{ route('trek-routes.index') }}?search={{ urlencode($t->name) }}" class="cat-pill">{{ $t->name }}</a>
                    @endforeach
                    @if($popularTreks->isEmpty())
                    <a href="{{ route('trek-routes.index') }}?search=Annapurna+Base+Camp" class="cat-pill">Annapurna Base Camp Trek</a>
                    <a href="{{ route('trek-routes.index') }}?search=Annapurna+Circuit" class="cat-pill">Annapurna Circuit Trek</a>
                    <a href="{{ route('trek-routes.index') }}?search=Pokhara" class="cat-pill">Pokhara City Tours</a>
                    <a href="{{ route('trek-routes.index') }}?search=Poon+Hill" class="cat-pill">Poon Hill Trek</a>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="stats-bar">
            <div class="row">
                <div class="col-6 col-md-3">
                    <div class="stat-item">
                        <h4>{{ $statTrekCount > 0 ? $statTrekCount.'+' : \App\Helpers\Cms::setting('home_stat1_value', '500+') }}</h4>
                        <p>{{ \App\Helpers\Cms::setting('home_stat1_label', 'Trek Routes') }}</p>
                    </div>
                </div>
                <div class="col-6 col-md-3">
                    <div class="stat-item">
                        <h4>{{ $statAgencyCount > 0 ? $statAgencyCount.'+' : \App\Helpers\Cms::setting('home_stat2_value', '200+') }}</h4>
                        <p>{{ \App\Helpers\Cms::setting('home_stat2_label', 'Travel Agencies') }}</p>
                    </div>
                </div>
                <div class="col-6 col-md-3">
                    <div class="stat-item">
                        <h4>{{ $statHotelCount > 0 ? $statHotelCount.'+' : \App\Helpers\Cms::setting('home_stat3_value', '150+') }}</h4>
                        <p>{{ \App\Helpers\Cms::setting('home_stat3_label', 'Hotels & Lodges') }}</p>
                    </div>
                </div>
                <div class="col-6 col-md-3">
                    <div class="stat-item">
                        <h4>{{ $statInquiryCount > 0 ? $statInquiryCount.'+' : \App\Helpers\Cms::setting('home_stat4_value', '50k+') }}</h4>
                        <p>{{ \App\Helpers\Cms::setting('home_stat4_label', 'Happy Trekkers') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="filter-drawer-body">
        ...
        <div class="filter-group">
            ...
            <div class="star-check-list">
                ...
                <label class="star-check-item">
                    ...
                    <span class="stars">@for($s=0;$s<$star;$s++)★@endfor</span>
                    <span>{{ $star }} Star</span>
                </label>
                @endforeach
            </div>
        ...
        <div class="filter-group">
            ...
            <div class="cat-check-list">
                @foreach(['Luxury','Standard','Budget','Eco Friendly','Community Based','Premium','Tented Camp','Environment Friendly','Cottage','Villa','Apartment'] as $cat)
                <label class="cat-check-item">
                    <input type="checkbox" name="category[]" value="{{ strtolower(str_replace(' ','-',$cat)) }}">
                    {{ $cat }}
                </label>
                @endforeach
            </div>
...
<section class="section-pad" id="dealsSection">
    <div class="container">
        <div class="sec-head">
            ...
            <div class="sec-nav">
                <a href="{{ route('search', ['type'=>'trekking']) }}" class="sec-view-all" id="dealsViewAll">View All <i class="ti-arrow-right"></i></a>
            ...
            <div class="owl-carousel owl-theme">
                ...
                <div class="deal-card">
                    <div class="deal-card-img">
                        @php $img = $trek->featured_image ? (str_starts_with($trek->featured_image,'annapurna/') ? asset($trek->featured_image) : \App\Helpers\Cms::imageUrl($trek->featured_image)) : asset('annapurna/img/tours/annapurna-circuit.jpg'); @endphp
                    ...
                    <div class="deal-card-body">
                        ...
                        <div class="deal-card-title"><a href="{{ route('trek-routes.show', $trek->slug) }}">{{ $trek->name }}</a></div>
                        <div class="deal-card-loc"><i class="ti-location-pin"></i> Gandaki Province, Nepal</div>
                        <div class="deal-card-footer"><div class="deal-card-price"><small>Starting From</small><strong>{{ $trek->price_range ?? 'On Request' }}</strong></div></div>
                        <div class="deal-card-meta">@if($trek->duration_days)<span><i class="ti-time"></i> {{ $trek->duration_days }} Days</span>@endif @if($trek->group_size_min)<span><i class="ti-user"></i> {{ $trek->group_size_min }}+</span>@endif</div>
                    </div>
                </div>
                @empty
                @endforelse
                ...
                <div class="deal-card">
                    <div class="deal-card-img">
                        @php $img = $hotel->featured_image ? \App\Helpers\Cms::imageUrl($hotel->featured_image) : asset('annapurna/img/hotels/temple-tree-resort-annapurna-region1.jpg'); @endphp
                        <img src="{{ $img }}" alt="{{ $hotel->name }}">
                        <span class="deal-badge verified">Hotel</span>
                    </div>
                    <div class="deal-card-body">
                        <div class="deal-card-rating"><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><span>5.0</span></div>
                        <div class="deal-card-title"><a href="{{ route('hotels.show', $hotel->slug) }}">{{ $hotel->name }}</a></div>
                        <div class="deal-card-loc"><i class="ti-location-pin"></i> {{ $hotel->city ?? $hotel->address ?? 'Pokhara, Nepal' }}</div>
                ...
                <div class="deal-card">
                    <div class="deal-card-img">
                        @php $img = $dest->featured_image ? \App\Helpers\Cms::imageUrl($dest->featured_image) : asset('annapurna/img/destination/pokhara-city-tours.png'); @endphp
                        <img src="{{ $img }}" alt="{{ $dest->name }}">
                        <span class="deal-badge hot">Destination</span>
                    </div>
                    <div class="deal-card-body">
                        <div class="deal-card-rating"><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><span>5.0</span></div>
                        <div class="deal-card-title"><a href="{{ route('destinations.show', $dest->slug) }}">{{ $dest->name }}</a></div>
                ...
                @endforeach
                @if($popularTreks->isEmpty() && $hotels->isEmpty() && $destinations->isEmpty())
                @php
                $staticDeals = [
                ...
                @endphp
                @foreach($staticDeals as $d)
                <div class="deal-card">
                    <div class="deal-card-img"><img src="{{ $d['img'] }}" alt="{{ $d['title'] }}"><span class="deal-badge {{ $d['badge'] }}">{{ $d['badge_label'] }}</span></div>
                    <div class="deal-card-body">
                        <div class="deal-card-rating"><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><span>5.0</span></div>
                        <div class="deal-card-title"><a href="{{ $d['url'] }}">{{ $d['title'] }}</a></div>
                        <div class="deal-card-loc"><i class="ti-location-pin"></i> {{ $d['loc'] }}</div>
                        <div class="deal-card-footer"><div class="deal-card-price"><small>Starting From</small><strong>{{ $d['price'] }}</strong></div></div>
                        <div class="deal-card-meta"><span><i class="ti-time"></i> {{ $d['days'] }} Days</span><span><i class="ti-user"></i> {{ $d['group'] }}</span></div>
                    </div>
                </div>
                @endforeach
                @endif
        ...
        <div class="deals-panel" id="panel-travel-agency">
            <div class="owl-carousel owl-theme">
                @forelse($travelAgencies as $agency)
                <div class="deal-card">
                    <div class="deal-card-img">
                        @php $img = $agency->featured_image ? \App\Helpers\Cms::imageUrl($agency->featured_image) : asset('annapurna/img/destination/pokhara-city-tours.png'); @endphp
                        <img src="{{ $img }}" alt="{{ $agency->name }}">
                        <span class="deal-badge verified">Agency</span>
                    </div>
                    <div class="deal-card-body">
                        <div class="deal-card-rating"><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><span>5.0</span></div>
                        <div class="deal-card-title"><a href="{{ route('businesses.show', $agency->slug) }}">{{ $agency->name }}</a></div>
                        <div class="deal-card-loc"><i class="ti-location-pin"></i> {{ $agency->city ?? $agency->address ?? 'Nepal' }}</div>
                        <div class="deal-card-footer"><div class="deal-card-price"><small>Starting From</small><strong>On Request</strong></div></div>
                        <div class="deal-card-meta"><span><i class="ti-car"></i> Travel Agency</span></div>
                    </div>
                </div>
                @empty
                <div class="deal-card"><div class="deal-card-img"><img src="{{ asset('annapurna/img/destination/pokhara-city-tours.png') }}" alt="Travel Agency"><span class="deal-badge verified">Agency</span></div><div class="deal-card-body"><div class="deal-card-rating"><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><span>5.0</span></div><div class="deal-card-title"><a href="{{ route('travel-agencies.index') }}">Explore Travel Agencies</a></div><div class="deal-card-loc"><i class="ti-location-pin"></i> Pokhara, Nepal</div><div class="deal-card-footer"><div class="deal-card-price"><small>Starting From</small><strong>On Request</strong></div></div><div class="deal-card-meta"><span><i class="ti-car"></i> Travel Agency</span></div></div></div>
                @endforelse
            </div>
        </div>
        ...
        <div class="deals-panel" id="panel-restaurant">
            <div class="owl-carousel owl-theme">
                <div class="deal-card"><div class="deal-card-img"><img src="{{ asset('annapurna/img/destination/pokhara-city-tours.png') }}" alt="Restaurants in Pokhara"><span class="deal-badge new" style="background:#e4a853;color:#0f2454;">Popular</span></div><div class="deal-card-body"><div class="deal-card-rating"><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><span>5.0</span></div><div class="deal-card-title"><a href="{{ route('restaurants.index') }}">Restaurants in Pokhara</a></div><div class="deal-card-loc"><i class="ti-location-pin"></i> Lakeside, Pokhara</div><div class="deal-card-footer"><div class="deal-card-price"><small>Starting From</small><strong>On Request</strong></div></div><div class="deal-card-meta"><span><i class="ti-cup"></i> Dining</span></div></div></div>
                <div class="deal-card"><div class="deal-card-img"><img src="{{ asset('annapurna/img/destination/paragliding-pokhara.jpg') }}" alt="Lakeside Dining"><span class="deal-badge verified">Verified</span></div><div class="deal-card-body"><div class="deal-card-rating"><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><i class="ti-star"></i><span>5.0</span></div><div class="deal-card-title"><a href="{{ route('restaurants.index') }}">Lakeside Dining Experience</a></div><div class="deal-card-loc"><i class="ti-location-pin"></i> Phewa Lake, Pokhara</div><div class="deal-card-footer"><div class="deal-card-price"><small>Starting From</small><strong>On Request</strong></div></div><div class="deal-card-meta"><span><i class="ti-cup"></i> Restaurant</span></div></div></div>
            </div>
        </div>
...
<section class="section-pad bg-light-gray">
    <div class="container">
        ...
        <div class="collection-grid">
            <a href="{{ route('trek-routes.index') }}" class="coll-card">
                <img src="{{ asset('annapurna/img/destination/annapurna-circuit1.jpg') }}" alt="Trekking Routes">
                <div class="coll-card-overlay"><h6>Trekking Routes</h6><span>12+ Routes</span></div>
            </a>
            <a href="{{ route('hotels.index') }}" class="coll-card">
                <img src="{{ asset('annapurna/img/hotels/temple-tree-resort-annapurna-region1.jpg') }}" alt="Luxury Stays">
                <div class="coll-card-overlay"><h6>Luxury Stays</h6><span>40+ Properties</span></div>
            </a>
            <a href="{{ route('destinations.index') }}" class="coll-card">
                <img src="{{ asset('annapurna/img/destination/paragliding-pokhara.jpg') }}" alt="Adventure Sports">
                <div class="coll-card-overlay"><h6>Adventure Sports</h6><span>8+ Activities</span></div>
            </a>
            <a href="{{ route('trek-routes.index') }}" class="coll-card">
                <img src="{{ asset('annapurna/img/destination/annapurna-base-camp-abc.png') }}" alt="Himalaya Treks">
                <div class="coll-card-overlay"><h6>Himalaya Treks</h6><span>7+ Destinations</span></div>
            </a>
            <a href="{{ route('destinations.index') }}" class="coll-card">
                <img src="{{ asset('annapurna/img/destination/ghandruk-village-annapurna.png') }}" alt="Cultural Heritage">
        ...
        <div class="owl-carousel owl-theme" id="listedCarousel">
            ...
            <div class="listed-card">
                <div class="listed-card-img">
                    ...
                    @endphp
                    <img src="{{ $hImg }}" alt="{{ $hotel->name }}">
                    <span class="deal-badge verified" style="top:8px;left:8px;font-size:9px;">Verified</span>
                </div>
                <div class="listed-card-body">
                    <h6><a href="{{ route('hotels.show', $hotel->slug) }}">{{ $hotel->name }}</a></h6>
                    <div class="listed-card-loc"><i class="ti-location-pin"></i> {{ $hotel->city ?? $hotel->location ?? 'Nepal' }}</div>
                    <div class="listed-card-footer">
                        <div class="listed-card-meta"><i class="ti-star"></i> {{ number_format($hotel->ranking_score ?? 5, 1) }}</div>
            ...
            <div class="listed-card">
                ...
                <div class="listed-card-body">
                    ...
                    <div class="listed-card-loc"><i class="ti-location-pin"></i> {{ $item['loc'] }}</div>
                    <div class="listed-card-footer">
                        <div class="listed-card-meta"><i class="ti-star"></i> {{ $item['rating'] }}</div>
                        <div class="listed-card-price"><small>From</small> <strong>{{ $item['price'] }}</strong></div>
                    </div>
                </div>
            </div>
            @endforeach
    ...
    <div class="container">
        <div class="cta-banner">
            <div class="cta-banner-img">
                <img src="{{ asset('annapurna/img/destination/transport-annapurna-region.jpg') }}" alt="Transport Service">
            </div>
            ...
            <div class="row">
                <div class="col-md-5">
                    <div class="country country1">
                        <div class="section-title2">{{ \App\Helpers\Cms::setting('home_trekking_title', 'Trekking in Nepal') }}</div>
                        ...
                        <div class="row tour-list">
                            <div class="col-md-6">
                                <ul>
                                    @php $treksLeft = $activeTreks->take(3); @endphp
                                    @forelse($treksLeft as $t)
                                    <li><i class="ti-location-pin"></i> <a href="{{ route('trek-routes.show', $t->slug) }}" class="link-btn">{{ $t->name }}</a></li>
                                    @empty
                            ...
                            <div class="col-md-6">
                                <ul>
                                    @php $treksRight = $activeTreks->skip(3)->take(3); @endphp
                                    @forelse($treksRight as $t)
                                    <li><i class="ti-location-pin"></i> <a href="{{ route('trek-routes.show', $t->slug) }}" class="link-btn">{{ $t->name }}</a></li>
                                    @empty
                        ...
                        <div class="butn-dark mt-30 mb-30"><a href="{{ route('trek-routes.index') }}"><span>Explore All Treks <i class="ti-arrow-right"></i></span></a></div>
                    </div>
                </div>
                <div class="col-md-7">
                    <div class="owl-carousel owl-theme">
                        <div class="item">
                            <div class="position-re o-hidden"><img src="{{ asset('annapurna/img/destination/annapurna-circuit1.jpg') }}" alt="Annapurna Circuit"></div>
                            <span class="category"><a href="{{ route('trek-routes.index') }}">$1,500–$3,000</a></span>
                            <div class="con"><h5><a href="{{ route('trek-routes.index') }}">Annapurna Circuit</a></h5><div class="line"></div><div class="row facilities"><div class="col col-md-12"><ul><li><i class="ti-time"></i> 7-15 Days</li><li><i class="ti-user"></i> 10+</li><li><i class="ti-location-pin"></i> Nepal</li></ul></div></div></div>
                        </div>
                        <div class="item">
                            <div class="position-re o-hidden"><img src="{{ asset('annapurna/img/destination/ghorepani-hill-trek.jpg') }}" alt="Ghorepani Hill Trek"></div>
                            <span class="category"><a href="{{ route('trek-routes.index') }}">$300–$1,300</a></span>
                            <div class="con"><h5><a href="{{ route('trek-routes.index') }}">Ghorepani Hill Trek</a></h5><div class="line"></div><div class="row facilities"><div class="col col-md-12"><ul><li><i class="ti-time"></i> 4-15 Days</li><li><i class="ti-user"></i> 8+</li><li><i class="ti-location-pin"></i> Nepal</li></ul></div></div></div>
                        </div>
                        <div class="item">
                            <div class="position-re o-hidden"><img src="{{ asset('annapurna/img/destination/mardi-himal-trek.jpg') }}" alt="Mardi Himal Trek"></div>
                            <span class="category"><a href="{{ route('trek-routes.index') }}">$800–$1,500</a></span>
                            <div class="con"><h5><a href="{{ route('trek-routes.index') }}">Mardi Himal Trek</a></h5><div class="line"></div><div class="row facilities"><div class="col col-md-12"><ul><li><i class="ti-time"></i> 6-9 Days</li><li><i class="ti-user"></i> 6+</li><li><i class="ti-location-pin"></i> Nepal</li></ul></div></div></div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="country country1">
                        ...
                        <div class="row tour-list">
                            <div class="col-md-6"><ul>
                                <li><i class="ti-location-pin"></i> <a href="{{ route('destinations.index') }}" class="link-btn">Phewa Lake Boating</a></li>
                                <li><i class="ti-location-pin"></i> <a href="{{ route('destinations.index') }}" class="link-btn">Paragliding</a></li>
                                <li><i class="ti-location-pin"></i> <a href="{{ route('destinations.index') }}" class="link-btn">Sarangkot Sunrise</a></li>
                            </ul></div>
                            <div class="col-md-6"><ul>
                                <li><i class="ti-location-pin"></i> <a href="{{ route('destinations.index') }}" class="link-btn">World Peace Pagoda</a></li>
                                <li><i class="ti-location-pin"></i> <a href="{{ route('destinations.index') }}" class="link-btn">Mountain Museum</a></li>
                                <li><i class="ti-location-pin"></i> <a href="{{ route('destinations.index') }}" class="link-btn">Devi's Falls</a></li>
                            </ul></div>
                        </div>
                        <div class="butn-dark mt-30 mb-30"><a href="{{ route('destinations.index') }}"><span>Explore Pokhara <i class="ti-arrow-right"></i></span></a></div>
                    </div>
                </div>
                <div class="col-md-7">
                    <div class="owl-carousel owl-theme">
                        <div class="item">
                            <div class="position-re o-hidden"><img src="{{ asset('annapurna/img/destination/pokhara-sightseeing.jpg') }}" alt="Pokhara Sightseeing"></div>
                            <span class="category"><a href="{{ route('destinations.index') }}">$350–$800</a></span>
                            <div class="con"><h5><a href="{{ route('destinations.index') }}">Pokhara Sightseeing</a></h5><div class="line"></div><div class="row facilities"><div class="col col-md-12"><ul><li><i class="ti-time"></i> 3-5 Days</li><li><i class="ti-user"></i> 10+</li><li><i class="ti-location-pin"></i> Nepal</li></ul></div></div></div>
                        </div>
                        <div class="item">
                            <div class="position-re o-hidden"><img src="{{ asset('annapurna/img/destination/paragliding-pokhara.jpg') }}" alt="Paragliding Pokhara"></div>
                            <span class="category"><a href="{{ route('destinations.index') }}">$120–$250</a></span>
                            <div class="con"><h5><a href="{{ route('destinations.index') }}">Paragliding in Pokhara</a></h5><div class="line"></div><div class="row facilities"><div class="col col-md-12"><ul><li><i class="ti-time"></i> 1-2 Days</li><li><i class="ti-user"></i> 2+</li><li><i class="ti-location-pin"></i> Nepal</li></ul></div></div></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-5">
                    <div class="country country1">
                        ...
                        <div class="row tour-list">
                            <div class="col-md-6"><ul>
                                <li><i class="ti-location-pin"></i> <a href="{{ route('destinations.index') }}" class="link-btn">Annapurna I (8,091m)</a></li>
                        ...
                        <div class="butn-dark mt-30 mb-30"><a href="{{ route('destinations.index') }}"><span>All Expeditions <i class="ti-arrow-right"></i></span></a></div>
                    </div>
                </div>
                <div class="col-md-7">
                    <div class="owl-carousel owl-theme">
                        <div class="item">
                            <div class="position-re o-hidden"><img src="{{ asset('annapurna/img/destination/machhapuchree-expedition.jpg') }}" alt="Machapuchare"></div>
                            <span class="category"><a href="{{ route('destinations.index') }}">$1,250–$2,500</a></span>
                            <div class="con"><h5><a href="{{ route('destinations.index') }}">Machapuchare Trek</a></h5><div class="line"></div><div class="row facilities"><div class="col col-md-12"><ul><li><i class="ti-time"></i> 7-14 Days</li><li><i class="ti-user"></i> 12+</li><li><i class="ti-location-pin"></i> Nepal</li></ul></div></div></div>
                        </div>
                        <div class="item">
                            <div class="position-re o-hidden"><img src="{{ asset('annapurna/img/destination/pisang-peak.jpg') }}" alt="Pisang Peak"></div>
                            <span class="category"><a href="{{ route('destinations.index') }}">$1,500–$2,500</a></span>
                            <div class="con"><h5><a href="{{ route('destinations.index') }}">Pisang Peak Trek</a></h5><div class="line"></div><div class="row facilities"><div class="col col-md-12"><ul><li><i class="ti-time"></i> 4-7 Days</li><li><i class="ti-user"></i> 6+</li><li><i class="ti-location-pin"></i> Nepal</li></ul></div></div></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-5 mb-30 mt-30">
                    ...
                    <h5>{{ \App\Helpers\Cms::setting('home_testimonials_heading', 'Reflections on the Majesty of Annapurna from Legends and Explorers') }}</h5>
                    @if($heroPhone)
                    <div class="phone-call mb-10">
                        <div class="icon color-1"><span class="flaticon-phone-call"></span></div>
                        <div class="text">
                            <p class="color-1">Call Now</p>
                            <a class="color-1" href="tel:{{ $heroPhone }}">{{ $heroPhone }}</a>
                        </div>
                    </div>
                    @endif
                            ...
                            <div class="item">
                                ...
                                <div class="info">
                                    <div class="author-img">
                                        <img src="{{ $testimonial->image ? \App\Helpers\Cms::imageUrl($testimonial->image) : asset('annapurna/img/team/08.png') }}" alt="{{ $testimonial->name }}">
                                    </div>
                                    <div class="cont">
                                        <div class="rating">@for($i=0;$i<5;$i++)<i class="star {{ $i<($testimonial->rating??5)?'active':'' }}"></i>@endfor</div>
            ...
            <div class="col-md-7">
                <div class="owl-carousel owl-theme">
                    ...
                    <div class="clients-logo">
                        @if($partner->url ?? null)
                            <a href="{{ $partner->url }}" target="_blank" rel="noopener noreferrer"><img src="{{ \App\Helpers\Cms::imageUrl($partner->logo) }}" alt="{{ $partner->name }}"></a>
                        @else
                            <img src="{{ \App\Helpers\Cms::imageUrl($partner->logo) }}" alt="{{ $partner->name }}">
                        @endif
                    </div>
                    @empty
...
$(document).ready(function () {
    ...
    var sliderImages = [
        ...
        @if($slider->image)
        { src: "{{ \App\Helpers\Cms::imageUrl($slider->image) }}" },
        @endif
        @endforeach
        @if($sliders->isEmpty())
        { src: "{{ asset('annapurna/img/slider/annapurna-region.png') }}" },
        { src: "{{ asset('annapurna/img/slider/panaromic.jpg') }}" }
        @endif
    ];
    ...
    var searchConfig = @json($searchConfig);

    var staticPills = {
        trekking: [
            @foreach($popularTreks->take(5) as $t)
            { label: '{{ $t->name }}', url: '{{ route("trek-routes.index") }}?search={{ urlencode($t->name) }}' },
            @endforeach
            @if($popularTreks->isEmpty())
            { label: 'Annapurna Base Camp Trek', url: '{{ route("trek-routes.index") }}?search=Annapurna+Base+Camp' },
            { label: 'Annapurna Circuit Trek',   url: '{{ route("trek-routes.index") }}?search=Annapurna+Circuit' },
            { label: 'Pokhara City Tours',       url: '{{ route("trek-routes.index") }}?search=Pokhara' },
            { label: 'Poon Hill Trek',           url: '{{ route("trek-routes.index") }}?search=Poon+Hill' },
            @endif
        ],
        hotels: [
            { label: 'Luxury Hotels',    url: '{{ route("hotels.index") }}?category=luxury' },
            { label: 'Budget Stays',     url: '{{ route("hotels.index") }}?category=budget' },
            { label: 'Pokhara Resorts',  url: '{{ route("hotels.index") }}?search=Pokhara' },
            { label: 'Lakeside Hotels',  url: '{{ route("hotels.index") }}?search=Lakeside' },
        ],
        travel: [
            { label: 'Day Tours',        url: '{{ route("travel-agencies.index") }}?search=day+tour' },
            { label: 'Adventure Sports', url: '{{ route("destinations.index") }}' },
            { label: 'Cultural Tours',   url: '{{ route("travel-agencies.index") }}?search=cultural' },
            { label: 'Paragliding',      url: '{{ route("destinations.index") }}?search=paragliding' },
        ],
        vehicle: [
            { label: 'Jeep Rental',      url: '{{ route("travel-agencies.index") }}?search=jeep' },
            { label: 'Bus Transfer',     url: '{{ route("travel-agencies.index") }}?search=bus' },
            { label: 'Private Car',      url: '{{ route("travel-agencies.index") }}?search=car' },
            { label: 'Airport Transfer', url: '{{ route("travel-agencies.index") }}?search=airport' },
        ],
    };
    ...
    var typeRoutes = {
        'all':          '{{ route("search", ["type"=>"trekking"]) }}',
        'trekking':     '{{ route("search", ["type"=>"trekking"]) }}',
        'hotel':        '{{ route("search", ["type"=>"hotel"]) }}',
        'travel-agency':'{{ route("search", ["type"=>"travel"]) }}',
        'destinations': '{{ route("search", ["type"=>"destinations"]) }}',
        'restaurant':   '{{ route("search", ["type"=>"restaurant"]) }}',
    };
    ...
    /* ----- Filter drawer ----- */
    var categoryRoutes = {
        'all':            '{{ route("trek-routes.index") }}',
        'trekking':       '{{ route("trek-routes.index") }}',
        'hotel':          '{{ route("hotels.index") }}',
        'travel-agency':  '{{ route("travel-agencies.index") }}',
        'destinations':   '{{ route("destinations.index") }}',
        'restaurant':     '{{ route("restaurants.index") }}'
    };
